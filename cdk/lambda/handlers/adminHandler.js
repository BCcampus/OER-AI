...
exports.handler = async (event) => {
  ...
  try {
    ...
    switch (pathData) {
      ...
      // DELETE /chat_sessions/{chat_session_id} - Delete specific chat session (admin or session owner)
      case "DELETE /chat_sessions/{chat_session_id}":
        ...
        // Determine if request is from an admin or a user
        const authorizer = event.requestContext?.authorizer || {};
        let isAdmin = false;
        try {
          const claims = authorizer.claims || {};
          const cognitoGroups = claims['cognito:groups'] || claims['groups'] || "";
          if (cognitoGroups && cognitoGroups.toLowerCase().includes('admin')) {
            isAdmin = true;
          }
        } catch (e) {
          // If claims are not present, fall back to non-admin path
        }

        if (!isAdmin) {
          // Non-admins must prove ownership by providing the matching user_session_id
          const providedUserSessionId =
            event.queryStringParameters?.user_session_id ||
            event.headers?.['x-user-session-id'] ||
            event.headers?.['X-User-Session-Id'];

          if (!providedUserSessionId) {
            response.statusCode = 400;
            response.body = JSON.stringify({
              error: "Missing user_session_id for owner verification",
            });
            break;
          }

          // Verify chat session exists and get owning user_session_id
          const ownerRes = await sqlConnection`
            SELECT user_session_id FROM chat_sessions WHERE id = ${chatSessionId}
          `;
          ...
          const ownerSessionId = ownerRes[0].user_session_id || ownerRes[0].user_sessions_session_id || ownerRes[0].userSessionId;
          if (!ownerSessionId || ownerSessionId.toString() !== providedUserSessionId.toString()) {
            response.statusCode = 403;
            response.body = JSON.stringify({ error: "Forbidden: Not the owner of this chat session" });
            break;
          }

          console.log(`Delete request for chat_session ${chatSessionId} by owner session ${providedUserSessionId}`);

          // Delete interactions for the user's chat session
          await sqlConnection`
            DELETE FROM user_interactions WHERE chat_session_id = ${chatSessionId}
          `;

          // Delete the chat session and return deleted ID to confirm operation
        ...
        // At this point, requester is admin
        console.log(`Delete request for chat_session ${chatSessionId} by admin ${authorizer.principalId || authorizer.claims?.email || 'unknown'}`);
